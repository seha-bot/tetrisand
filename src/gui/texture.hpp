...
class Texture : public Grid<uint32_t>
{
  public:
    ...
    Texture(const std::vector<uint32_t>& texture,
            uint32_t width,
...
class Color final
{
  ...
  public:
    ...
    {
        return std::make_tuple((color_ << 16 & 0xFF) / 255.0,
                               (color_ << 8 & 0xFF) / 255.0,
                               (color_ & 0xFF) / 255.0);
    }
    ...
    Color(std::tuple<double, double, double> color) noexcept
      : color_(static_cast<uint32_t>(std::clamp(std::get<0>(color), 0.0, 1.0) *
                                     255.0)
                 << 16 |
               static_cast<uint32_t>(std::clamp(std::get<1>(color), 0.0, 1.0) *
                                     255.0)
                 << 8 |
               static_cast<uint32_t>(std::clamp(std::get<2>(color), 0.0, 1.0) *
                                     255.0))
    {
    }
};
...
class PostProcessedTexture : public Texture
{
  ...
  public:
    ...
    PostProcessedTexture ror() noexcept
    {
        return { Texture::ror(), processor_ };
    }
