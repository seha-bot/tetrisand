...
void
DrawCleaner::setPixel(int32_t x, int32_t y, uint32_t color)
{
    // SDL_PixelFormat pixelFormat;
    // pixelFormat.format = SDL_PIXELFORMAT_RGB24;
    // pixels[x + y * tH.width] = SDL_MapRGB(&pixelFormat, 100, 100, 100);
    pixels[x + y * tH.window.width()] = color;
}
...
Drawable::Drawable(Window& window, uint32_t width, uint32_t height)
  : tH(new TextureHandle{ nullptr, window },
       [](TextureHandle* handle) { SDL_DestroyTexture(handle->texture); })
{
